...
#Plot
ggplot(data = summary_data_final %>% group_by(time_point, treatment, replicate) %>% 
...
summary_data_percentiles <- summary_data_final %>%
  ...
  mutate(time_point = time_point - 1) %>% ungroup() %>% group_by(treatment) %>% 
...
#clean data to analyze (we select the cariables we interested in)
data_prey_clean<-summary_data_percentiles %>%
...
#visualize
ggplot(data = data_prey_clean %>% group_by(time_point, treatment, replicate, Species) %>%
...
#write csv
write.csv(data_prey_clean, "prey_data_IDs_clean.csv", row.names = FALSE)
saveRDS(data_prey_clean, file = "prey_data_IDs_clean.RDS")
